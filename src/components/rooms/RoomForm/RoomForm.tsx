...
export default function RoomForm({ save, close }: RoomFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4 space-y-4 bg-white rounded-md shadow-md dark:bg-gray-900 dark:text-white">
      ...
      {/* Room No */}
      <div className="w-full md:w-1/2">
        <label className="block mb-1 text-sm">Room No</label>
        <input
          type="number"
          {...register("room_no", {
            required: "Room no. is required",
            min: { value: 1, message: "Room no. must be greater than 0" },
          })}
          className="w-full border border-gray-300 p-2 rounded-lg dark:border-gray-600 dark:bg-gray-800"
          placeholder="Room no."
        />
        {errors.room_no && <p className="text-red-400 text-sm">{errors.room_no.message}</p>}
      </div>
      ...
      {/* Room Description */}
      <div className="w-full md:w-1/2">
        <label className="block mb-1 text-sm">About Room</label>
        <textarea 
